...
Object_FromUpdateCollectionModule(cr_UpdateCollectionModule *mod)
{
    ...
    py_rec = PyObject_CallObject((PyObject *)get_cr_module_state_global()->UpdateCollectionModule_Type, NULL);
    if (!py_rec)
        return NULL;
...
int
UpdateCollectionModuleObject_Check(PyObject *o)
{
    ...
    return PyObject_TypeCheck(o, state->UpdateCollectionModule_Type);
}
...
static int
updatecollectionmodule_traverse(PyObject *op, visitproc visit, void *arg)
{
    Py_VISIT(Py_TYPE(op));
    return 0;
}
...
set_uint(_UpdateCollectionModuleObject *self, PyObject *value, void *member_offset)
{
    ...
    if (PyLong_Check(value)) {
        ...
        val = (guint64) PyFloat_AsDouble(value);
    } else {
...
static PyType_Slot UpdateCollectionModule_Type_slots[] = {
    {Py_tp_traverse, updatecollectionmodule_traverse},
