...
queue_package_if_finished(cr_Package *pkg, cr_CbData *cb_data, GError **err)
{
        ...
        (pkg->loadingflags & CR_PACKAGE_LOADED_FIL))
    {
        if (cb_data->in_progress_pkgs_hash) {
            ...
            g_slist_delete_link(cb_data->in_progress_pkgs_list, cb_data->in_progress_pkgs_list);
        }
...
// TODO: double check that this doesn't break the traditional APIs and finished_pkgs_queue doesn't leak memory in that case
// If this code is running at all for the traditional APIs
                                        ...
                                        GError **err)
{
    ...
    cr_PkgIterator* pkg_iterator = cr_PkgIterator_new(
        primary_path, filelists_path, other_path, newpkgcb, newpkgcb_data, warningcb, warningcb_data, allow_out_of_order, err
    );
    ...
    while (package = cr_PkgIterator_parse_next(pkg_iterator, err)) {
        ...
        if (pkgcb && pkgcb(package, pkgcb_data, &tmp_err)) {
            if (tmp_err) {
    ...
    assert(cr_PkgIterator_is_finished(pkg_iterator) || !*err);
...
void cr_PkgIterator_free(cr_PkgIterator *iter, GError **err) {
    ...
    cr_CbData *cbdata = iter->cbdata;  // avoid dereferencing void *
